...
import { Bot, ClipboardCheck, Globe2, LogIn, LogOut, Moon, PlusCircle, RefreshCw, Send, ShieldCheck, Sun, UserRound, Workflow, X } from "lucide-react";
...
function localizeAuthError(error, isArabic, authMode) {
  const message = error instanceof Error ? String(error.message || "").trim() : "";

  if (!message || message === "Request failed." || message === "Request failed" || message === "Failed to fetch") {
    return isArabic
  ...
  if (message === "Incorrect email or password.") {
    return isArabic ? "البريد الإلكتروني أو كلمة المرور غير صحيحة." : message;
  }

  if (message === "This account has been blocked. Please contact the platform administrator.") {
    return isArabic ? "هذا الحساب محظور. يرجى التواصل مع مدير المنصة." : message;
  }

  if (message === "Email and password are required.") {
    return isArabic ? "يرجى إدخال البريد الإلكتروني وكلمة المرور." : message;
  }

  if (message === "Name, email, and password are required.") {
    return isArabic ? "يرجى إدخال الاسم والبريد الإلكتروني وكلمة المرور." : message;
  }

  if (message === "An account with this email already exists.") {
    return isArabic ? "يوجد حساب مسجل بهذا البريد الإلكتروني." : message;
  }
...
export default function PlatformApp() {
  ...
  async function handleAuthSubmit(event) {
    ...
    try {
      ...
      setNotice({ type: "error", text: localizeAuthError(error, isArabic, authMode) });
    } finally {
      setBusy("");
    }
  }
  ...
  async function handleNotificationDismiss(notificationId) {
    try {
      await api.dismissNotification(token, notificationId);
      setNotifications((current) => current.filter((item) => item.id !== notificationId));
    } catch {
      // Keep UX light for dismiss actions.
    }
  }
  ...
  return (
    <AppShell brandLogo={brandLogo} navigation={navigation} themeStyle={themeStyle(themeTokens, navigation.length)} backgroundMotif={backgroundMotif}>
      ...
      <main className="content-shell">
        {notice && <NoticeBanner notice={notice} />}
        {page === "home" && <HomePage dashboard={dashboard} text={text} user={user} canCreateChallenges={canCreateChallenges} canReviewIdeas={canReviewIdeas} notifications={notifications} onNotificationRead={handleNotificationRead} onNotificationDismiss={handleNotificationDismiss} />}
        {page === "challenges" && (
          <ChallengesPage
...
function HomePage({ dashboard, text, user, canCreateChallenges, canReviewIdeas, notifications, onNotificationRead, onNotificationDismiss }) {
  return (
    <>
            ...
            {notifications?.length ? (
              <div className="activity-list">
                {notifications.map((notification) => (
                  <div key={notification.id} className={`activity-row notification-row ${notification.isRead ? "read" : "unread"}`}>
                    <button type="button" className="notification-main" onClick={() => onNotificationRead(notification.id)}>
                    ...
                    <button
                      ...
                      aria-label={text.meta.dir === "rtl" ? "إخفاء الإشعار" : "Dismiss notification"}
                    >
                      <X size={16} />
                    </button>
                  </div>
